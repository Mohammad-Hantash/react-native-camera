...
>>>>>>> 57bc327d847ccb7ea90ded51a5a82168388b1349:ios/RCT/RCTCameraManager.m
}
...
RCT_EXPORT_METHOD(getCurrentColor:(RCTPromiseResolveBlock)resolve
                  reject:(RCTPromiseRejectBlock)reject) {
     if(_currentColor){
        resolve(_currentColor);
    }
}
...
- (void)startSession {
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        if(self.enableColorExtraction){
                AVCaptureVideoDataOutput * videoDataOutput = [[AVCaptureVideoDataOutput alloc ] init];
            if ([self.session canAddOutput:videoDataOutput])
            { 
                [videoDataOutput setSampleBufferDelegate:self queue:self.sessionQueue];
                [videoDataOutput setVideoSettings:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithUnsignedInt:kCVPixelFormatType_32BGRA], (id)kCVPixelBufferPixelFormatTypeKey, nil]];
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection{
    [self colorFromSampleBuffer:sampleBuffer];
}
...
- (void) colorFromSampleBuffer:(CMSampleBufferRef) sampleBuffer
{
    CVPixelBufferRef imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer);
    CVPixelBufferLockBaseAddress(imageBuffer,0);
    _tempAddress = (uint8_t *)CVPixelBufferGetBaseAddress(imageBuffer);
    ...
    int x = ((int)width)/2;
    int y = ((int)height)/2;
    int bufferSize = ((int)bytesPerRow) * height;
    
    uint8_t *myPixelBuf = malloc(bufferSize);
    memmove(myPixelBuf, _tempAddress, bufferSize);
    _tempAddress = nil;
    ...
    free(myPixelBuf);
    [self updateCurrentColor:r green:g blue:b];
}

- (void)updateCurrentColor:(int)red green:(int)green blue:(int)blue {
    _extractedColor = [NSString stringWithFormat:@"#%02lX%02lX%02lX",
            lroundf(red),
            lroundf(green),
            lroundf(blue)];
    NSLog(@"Color %@",_extractedColor);
    if(![_currentColor isEqualToString:_extractedColor]){
        _currentColor = _extractedColor;
        [self.camera updatePreviewColor:red green:green blue:blue];
    }
 }
